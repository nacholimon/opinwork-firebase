export const esMX = {
    // Common
    loading: 'Cargando...',
    email: 'Correo electrónico',
    password: 'Contraseña',
    signIn: 'Iniciar sesión',
    signOut: 'Cerrar sesión',
    signInWithGoogle: 'Iniciar sesión con Google',
    error: 'Error',
    save: 'Guardar',
    cancel: 'Cancelar',
    edit: 'Editar',
    name: 'Nombre',
    phone: 'Teléfono',
    profile: 'Mi Perfil',
    manageYourProfile: 'Gestiona tu información personal',
    updateProfile: 'Actualizar Perfil',
    profileUpdated: 'Perfil actualizado correctamente',
    profileUpdateError: 'Error al actualizar el perfil',
    optional: 'Opcional',
    notSet: 'No establecido',
    editProfile: 'Editar Perfil',
    photoUpdated: 'Foto de perfil actualizada correctamente',
    photoUpdateError: 'Error al actualizar la foto de perfil',
    theme: 'Tema',
    darkMode: 'Modo Oscuro',
    lightMode: 'Modo Claro',
    systemTheme: 'Usar tema del sistema',
    themeUpdated: 'Tema actualizado correctamente',
    themeUpdateError: 'Error al actualizar el tema',
    lastLogin: 'Último acceso',

    // Home
    welcome: 'Bienvenido a Mi Aplicación',
    subtitle: 'Una aplicación web moderna construida con React y Firebase',
    getStarted: 'Comenzar',

    // Navbar
    home: 'Inicio',
    dashboard: 'Panel de control',
    adminUsers: 'Administrar Usuarios',

    // Login
    signInToAccount: 'Inicia sesión en tu cuenta',

    // FirstAdmin
    createFirstAdmin: 'Crear primera cuenta de administrador',
    firstAdminSubtitle: 'Esta será la primera cuenta de administrador del sistema',
    createAdminAccount: 'Crear cuenta de administrador',

    // Dashboard
    emailAddress: 'Correo electrónico',
    accountCreated: 'Cuenta creada',
    user: 'Usuario',

    // AdminDashboard
    adminDashboard: 'Panel de administración',
    adminWelcome: '¡Bienvenido, administrador! Aquí puedes gestionar usuarios y realizar tareas administrativas.',

    // AdminUsers
    users: 'Usuarios',
    invitations: 'Invitaciones',
    role: 'Rol',
    actions: 'Acciones',
    deactivate: 'Desactivar',
    activate: 'Activar',
    expirationDays: 'Días de expiración',
    day: 'día',
    days: 'días',
    generateInvitation: 'Generar Invitación',
    invitationLink: 'Link de Invitación',
    createdAt: 'Creado el',
    expiresAt: 'Expira el',
    status: 'Estado',
    used: 'Usado',
    expired: 'Expirado',
    active: 'Activo',
    deactivated: 'Inactivo',
    copy: 'Copiar',
    manageUsersAndInvitations: 'Gestiona usuarios e invitaciones del sistema',
    userActivated: 'Usuario activado correctamente',
    userDeactivated: 'Usuario desactivado correctamente',
    userStatusUpdateError: 'Error al actualizar el estado del usuario',
    userUpdated: 'Usuario actualizado correctamente',
    userUpdateError: 'Error al actualizar el usuario',
    invitationGenerated: 'Invitación generada correctamente',
    invitationGenerationError: 'Error al generar la invitación',
    linkCopied: 'Link copiado al portapapeles',

    // Dashboard Tools
    creditSimulator: 'Simulador de Crédito',
    creditSimulatorDescription: 'Simula tu crédito de manera sencilla y rápida.',
    housingCatalog: 'Catálogo de Viviendas',
    housingCatalogDescription: 'Explora las viviendas disponibles en nuestro catálogo.',
    userManagement: 'Gestión de Usuarios',
    userManagementDescription: 'Administra los usuarios y sus permisos en la plataforma.',
}; 